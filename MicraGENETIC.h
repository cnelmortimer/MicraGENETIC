...
template <class T> void MicraGENETIC<T>::escogerReproducirMutar(){///printf("#Población:\n");for(int i = 0; i<nIndividuos; i++){poblacion[i].verVariables();}
    ...
    for(int i = 0; i<nReproductores; i+=2){//Desenrollado manual. Prescindiendo de una funcion de busqueda adicional
        ...
        T descendiente = poblacion[focoSeleccionA].reproducir(poblacion[focoSeleccionB]);///printf("#Descendiente original:"); descendiente.verVariables();
        if(drand48()<probabilidadMutacion){//¿Solicitar mutacion?
            descendiente.mutar();///printf("#Descendiente mutado:"); descendiente.verVariables();
        }
        this->descendencia[nSeleccionados] = descendiente;
        nSeleccionados++;
    }
